...
BASE_LIBS="server.hoon skeleton.hoon default-agent.hoon dbug.hoon docket.hoon"
BASE_LIBS="$BASE_LIBS test.hoon"
...
if [[ "$USE_BASE" -eq 1 ]]; then
  ...
  for lib in $BASE_LIBS; do
    [[ -f "$BASE/lib/$lib" ]] && copy_file "$BASE/lib/$lib" "$OUT/lib/$lib"
  done
  ...
  [[ -f "$BASE/sys.kelvin" ]] && copy_file "$BASE/sys.kelvin" "$OUT/sys.kelvin"
fi
